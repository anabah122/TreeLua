...
local PointLightFaces = require("engine.lights.PointLight").FACES
local Color     = require "engine.math.color"
local Vector3   = require "engine.math.vec3"
local Frustum   = require "engine.math.frustum"
local ShaderLib = require "engine.shader"
local getParticleShader = require "engine.shader.particles"
...
function WebGLRenderer:_sendShadowUniforms(sh, casters)
    ...
    local settings = require "engine.core.settings"
...
function WebGLRenderer:_renderDepthPass(scene, viewProj, cube, lightPos, far, wantMovable)
    local getDepthShader = require "engine.shader.depth"
...
function WebGLRenderer:_compositeShadowMin(staticCanvas, dynamicCanvas, target, size, faceBasis)
    local getCompositeShader = require "engine.shader.shadowcomposite"
