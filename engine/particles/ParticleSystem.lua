...
local function buildShapeMesh(shape)
    ...
    local geo
    if shape == "box" then
        geo = require("engine.geometries.BoxGeometry"):new(1, 1, 1)
    elseif shape == "sphere" then
        geo = require("engine.geometries.SphereGeometry"):new(0.5, 12, 8)
    else
...
function ParticleSystem:new(params)
    ...
    p.direction = params.direction and params.direction:clone()
                  or require("engine.math.vec3"):new(0, 1, 0)
    ...
    p.gravity = params.gravity and params.gravity:clone()
                or require("engine.math.vec3"):new(0, 0, 0)
