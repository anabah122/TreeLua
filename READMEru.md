...
`Sprite` — квад, чей мировой *поворот* рендерер каждый кадр перезаписывает,
чтобы он смотрел на камеру; позиция/масштаб — как у обычного `Mesh`:
...
`DirectionalLight` умеет отбрасывать одну карту теней (один источник — одна
карта, см. [Ограничения](#ограничения)):

```lua
sun.castShadow = true          -- включить теневой проход для этого света
mesh.castShadow = true         -- на каждый меш, как в three.js
floor.receiveShadow = true
```

`renderer = TL.WebGLRenderer:new{ shadowTarget = TL.Vector3:new(0,0,0) }`
задаёт точку, вокруг которой теневая камера центрируется каждый кадр (обычно
центр сцены или игрок). `TL.settings.shadowSoftness` (по умолчанию `1`) —
радиус PCF-ядра в текселях, глобальный, как возможность GPU, а не свойство
...
| `demos/` | сцены, построенные на движке как на обычной библиотеке — не часть движка, используются для сквозной проверки |
...
`main.lua` делегирует в одну из сцен `demos/` (сейчас — `demos/shadows`);
каждая построена на TreeEngine строго как на библиотеке: ни один файл движка
не меняется, чтобы это работало, так что демо заодно служит интеграционной
проверкой:

| Демо | |
|---|---|
| `demos/shadows` | shadow mapping от направленного света, мягкость PCF, анимированный источник тени |
| `demos/isaac` | небольшой top-down шутер — комнаты, враги, пикапы, billboard-спрайты |

Они не часть движка и не место для примеров API сверх того, что уже
приведено выше; смотрите `init.lua` конкретного демо, если хотите увидеть
полную сборку сцены.
